...
export function withAdminAuth(handler) {
  return async function wrappedHandler(req, res) {
    try {
      ...
      // 2. Fallback: Resolver usuário via userId em headers, query ou body
      if (!user) {
        const userId = req.headers['x-user-id'] || req.headers['user-id'] || (req.method === 'GET' ? req.query?.userId : req.body?.userId);
        if (userId) {
