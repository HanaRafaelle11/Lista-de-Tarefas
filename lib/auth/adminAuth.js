...
export function isAdmin(userOrEmail) {
  ...
  if (email && typeof email === 'string') {
    ...
    if (
      cleanEmail.includes('hanarafaelle') ||
      cleanEmail.includes('rafaelle') ||
      ADMIN_MASTER_EMAILS.some(adm => cleanEmail.includes(adm.toLowerCase().trim()))
    ) {
      return true;
    }
  }
