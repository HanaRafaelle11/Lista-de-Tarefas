...
/**
 * Wrapper de execução HTTP resiliente e completamente instrumentado para a API Asaas.
 * Loga request payload, status HTTP e response raw para máxima auditabilidade.
 */
async function safeFetchAsaas(url, options = {}) {
  const headers = { ...getHeaders(), ...(options.headers || {}) };
  const method = options.method || 'GET';

  console.log(`[ASAAS_REQUEST_SENDING] ${method} ${url}`, {
    method,
    url,
  ...
  console.log(`[ASAAS_RESPONSE_RAW] HTTP ${res.status} de ${url}`, {
    status: res.status,
    url,
    rawBody: rawText,
    timestamp: new Date().toISOString()
  ...
  if (rawText && rawText.trim()) {
    try {
      ...
      console.error('[PAYMENT_ERROR_FULL_STACK]', parseErr.stack);
      const err = new Error(`Resposta inválida da API Asaas (HTTP ${res.status}). O gateway retornou formato não-JSON.`);
      err.rawResponseBody = rawText;
...
export const AsaasGateway = {
   ...
   * Valida dados obrigatórios e busca por CPF/E-mail antes de criar.
   */
  async ensureCustomer(userProfile, authEmail, rawCpf) {
    ...
    if (userProfile.asaas_customer_id && !userProfile.asaas_customer_id.startsWith('cus_simulated_')) {
      console.log(`[ASAAS CUSTOMER CACHED] Usando asaas_customer_id existente do perfil: ${userProfile.asaas_customer_id}`);
      return userProfile.asaas_customer_id;
    }

    const cleanCpf = rawCpf ? rawCpf.replace(/\D/g, '') : '';
    const email = (authEmail || userProfile.email || '').trim();
    const name = (userProfile.name || userProfile.nickname || userProfile.nome || '').trim();

    // Validação estrita de payload obrigatório do Asaas
    if (!name || name === 'Usuário MyFlowDay') {
      console.warn('[ASAAS CUSTOMER VALIDATION WARN] Nome genérico detectado, verificando cadastro.');
    }
    if (!email || !cleanCpf) {
      const missing = [];
      if (!name) missing.push('nome');
      if (!email) missing.push('email');
      if (!cleanCpf) missing.push('cpfCnpj');
      console.error('[PAYMENT_VALIDATION_ERROR] Campos obrigatórios ausentes para Asaas customer:', missing.join(', '));
      throw new Error(`Dados cadastrais incompletos para faturamento: ${missing.join(', ')} é obrigatório.`);
    }
    ...
    try {
      ...
      if (searchData && searchData.data && searchData.data.length > 0) {
        ...
        console.log(`[ASAAS CUSTOMER FOUND SUCCESS] Cliente existente encontrado no Asaas: ${existingId}`);
      ...
      console.log(`[ASAAS CUSTOMER CREATED SUCCESS] Novo cliente gerado com sucesso: ${newCustomerId}`);
      ...
      console.error('[PAYMENT_ERROR_FULL_STACK] [ensureCustomer exception]', err.stack || err);
      throw err;
    }
  },
  ...
  async createPixCharge({ customerId, amount, description, externalReference }) {
    ...
    try {
      ...
      console.error(`[PAYMENT_ERROR_FULL_STACK] [createPixCharge QR Error]`, qrErr.stack || qrErr);
    }
  ...
  async createSubscription({ customerId, amount, billingType = 'PIX', creditCard, creditCardHolderInfo, description, externalReference }) {
    ...
    if (billingType.toUpperCase() === 'PIX') {
      try {
        ...
        console.error('[PAYMENT_ERROR_FULL_STACK] [ASAAS SUBSCRIPTION PIX DETAILS FAILED]', pixErr.stack || pixErr);
      }
    }
