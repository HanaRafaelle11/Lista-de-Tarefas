...
export const BillingStateMachine = {
  ...
  /**
   * Efetua a transição de estado. Se for inválida, bloqueia e retorna o estado atual.
   */
  transition(currentStatus, newStatus) {
    ...
    if (!this.isValidTransition(current, next)) {
      console.error(`[BillingStateMachine] BLOCKED invalid transition: ${current} -> ${next}. Keeping current state.`);
      return current;
    }
    return next;
  }
