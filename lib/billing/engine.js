...
export const BillingEngine = {
  ...
  async processPaymentSuccess({ userId, customerId, paymentId, subscriptionId, billingType = 'pix', value, periodDays = 30 }) {
    ...
    return await DistributedLock.withLock('subscription:' + userId, async () => {
      ...
      // PILAR 1: EVENT SOURCING (Append-Only Event Registration)
      try {
        await supabaseAdmin.from('billing_events').insert([{
          event_id: `evt_sub_activated_${paymentId || Date.now()}_${userId}`,
          user_id: userId,
        ...
        console.warn('[BillingEngine] Event Sourcing append warning:', evtErr.message);
      }
