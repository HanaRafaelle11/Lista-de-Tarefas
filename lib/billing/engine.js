...
export const BillingEngine = {
  ...
  async processPaymentSuccess({ userId, customerId, paymentId, subscriptionId, billingType = 'pix', value, periodDays = 30 }) {
    ...
    return await DistributedLock.withLock('subscription:' + userId, async () => {
      ...
      // UNIFIED BILLING PIPELINE: EVENT SOURCING & LEDGER (Append-Only)
      try {
        const idempotencyKey = paymentId || `pay_proc_${userId}_${now.getTime()}`;
        const isAlreadyProcessed = await idempotencyStore.isProcessed(idempotencyKey);
        ...
        if (!isAlreadyProcessed) {
          ...
          await supabaseAdmin.from('billing_events').insert([{
            ...
            asaas_payment_id: idempotencyKey,
            value: value || priceVal,
            amount: value || priceVal,
            provider: PROVIDER_ASAAS,
          ...
          console.log(`[BillingEngine] Idempotency Hit for key ${idempotencyKey}. Skipping duplicate ledger entry.`);
        }
      } catch (evtErr) {
