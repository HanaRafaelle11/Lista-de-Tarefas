...
import { BillingEngineV2 } from '../../server/modules/billing/v2/billing.engine.v2.js';
...
export const BillingEngine = {
  ...
  async processPaymentSuccess({ userId, customerId, paymentId, subscriptionId, billingType = 'pix', value, periodDays = 30 }) {
    ...
    return await DistributedLock.withLock('subscription:' + userId, async () => {
      ...
      console.log('[BILLING ACTIVATING USER]', userId, 'NEW STATUS:', nextStatus, 'EXPIRES:', expiryDate.toISOString());
      ...
      // FIX 1 & 2: Update direcionado na linha existente + fallback para upsert/insert
      ...
      if (updateErr || !updateData || updateData.length === 0) {
        console.log('[BILLING UPDATE FALLBACK] Linha pendente não encontrada para update direto, executando upsert:', userId);
      ...
      AccessDecisionEngine.invalidateCache(userId);
      try {
        const { invalidateDashboardCache } = await import('../../services/admin-dashboard-service.js');
        invalidateDashboardCache();
      } catch (cacheErr) {
        console.warn('[BillingEngine] Invalidate Dashboard Cache warning:', cacheErr.message);
      }
      ...
      // PILAR 1: EVENT SOURCING & LEDGER (Append-Only Event Registration)
      try {
        const ledgerId = `evt_sub_activated_${paymentId || Date.now()}_${userId}`;
        ...
        console.warn('[BillingEngine] Ledger append warning:', evtErr.message);
      }

      // SHADOW MODE V2 COEXISTENCE (Sem interromper V1)
      try {
        BillingEngineV2.process({
          eventId: `evt_v2_shadow_${paymentId || Date.now()}`,
          eventType: 'payment_approved',
          payload: { userId, paymentId, customerId, billingType, value: value || priceVal },
          source: 'shadow_mode',
          idempotencyKey: paymentId || `pay_shadow_${userId}`
        }).catch(err => console.warn('[BillingEngineV2 Shadow Error]', err.message));
      } catch (shadowErr) {
        console.warn('[BillingEngineV2 Shadow Exception]', shadowErr.message);
      }
