  ...
  const handleSubmit = useCallback(async (param) => {
    try {
      ...
      if (resData.status === 'pending' || resData.status === 'created') {
        ...
        setError('O Mercado Pago recusou ou cancelou a geração deste Pix. Certifique-se de não estar usando o CPF do dono da conta do Mercado Pago para testar.');
        setStatus('error');
      } else {
        // Se cair aqui, realmente foi aprovado de alguma outra forma
        setStatus('success');
      }
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ opacity: isFormValid ? 1 : 0.15, filter: isFormValid ? 'none' : 'blur(4px)', transition: 'all 0.3s ease' }}>
                  ...
                  <div style={{
                    ...
                    <button
                      onClick={() => isFormValid && setActiveTab('card')}
                      style={{
                        flex: 1,
                        padding: '12px 10px',
                        borderRadius: '8px',
                ...
                {/* Blurred lock overlay blocking the entire payment methods section (visual gate) */}
                {!isFormValid && (
                  <div style={{
                    position: 'absolute',
                    top: 0,
